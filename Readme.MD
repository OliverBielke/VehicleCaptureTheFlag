...
The PacMan environment is built using a custom synchronized HTTP server, on top of Protobuf messages. 
...
This can be started by running the environment with "Mode: Local" in PacManManagerModeSelector or by disabling the mode selector entirely.
...
The game is **server authoritative.** All data that is not supposed to be seen by the Client does not exist on the Client.

The game is **fully synchronous**. The server waits for both clients to send in their actions before stepping the environment.
...
To play back a replay, **Disable the GameMode (PacManManagerModeSelector) object** and **enable the ReplayManager (GameStatePlaybackManager)**. 
Select a file and start the game in the editor. 
